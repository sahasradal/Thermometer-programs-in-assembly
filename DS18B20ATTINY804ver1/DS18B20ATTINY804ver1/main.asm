;
; DS18B20ATTINY804ver1.asm
;
; Created: 09/04/2022 17:16:55
; Author : Manama
; 
...
;For CRC check rhight shift the data , check carry, if set XOR lsb register with polynomial else shift data without XOR
;do this 56 times to get CRC. If data is with CRC the 56th shift will leave LSB register with 0 if transmission was good
...
reset:
ldi r16, 0xff
sts SCRATCHPAD + 7 ,r16
ldi r16, 0xff
sts SCRATCHPAD + 6,r16
ldi r16, 0xff
sts SCRATCHPAD + 5,r16
ldi r16, 0xff
sts SCRATCHPAD + 4,r16
ldi r16, 0x00
sts SCRATCHPAD + 3,r16
ldi r16, 0x00
sts SCRATCHPAD + 2,r16
ldi r16, 0x00
sts SCRATCHPAD + 1,r16
ldi r16, 0x72
sts SCRATCHPAD  ,r16
	push r8
	push r7
...
check1:
	...
	rcall byte_read				; call read_byte subroutine to read scratch pad ( total 9 bytes with lsbyte and lsbit first transmitted from sensor, readbyte rotates data so that lsbit 1st received is stored as lsb and last bit is stored as msb))		
	sts SCRATCHPAD + 7, r24		; store LSB available in r24 to SRAM register SCRATCHPAD+7
	rcall byte_read				; read LSB + 1 from sensor
	sts SCRATCHPAD + 6, r24		; store LSB available in r24 to SRAM register SCRATCHPAD+6
	rcall byte_read				; read LSB + 2 from sensor
	sts SCRATCHPAD + 5, r20		; store LSB available in r24 to SRAM register SCRATCHPAD+5
	rcall byte_read				; read LSB + 3 from sensor
	sts SCRATCHPAD + 4, r24		; store LSB available in r24 to SRAM register SCRATCHPAD+4
	rcall byte_read				; read LSB + 4 from sensor
	sts SCRATCHPAD + 3, r24		; store LSB available in r24 to SRAM register SCRATCHPAD+3
	rcall byte_read				; read LSB + 5 from sensor
	sts SCRATCHPAD + 2, r24		; store LSB available in r24 to SRAM register SCRATCHPAD+2
	rcall byte_read				; read LSB + 6 from sensor
	sts SCRATCHPAD + 1, r24		; store LSB available in r24 to SRAM register SCRATCHPAD+1
	rcall byte_read				; read MSB from sensor(contains CRC of ROM or scratchpad)
	sts SCRATCHPAD , r24		; store MSB available in r24 to SRAM register SCRATCHPAD
	rcall DALLAS_INIT			; call sensor init is required before and after each operation
	rcall checkCRC				; call subroutine checkCRC to check validity of data received
	tst r8
	brne measure
...
bit_read:
	ldi r25,1			; subroutine used to read 1 bit from sensor, so set counter to 1 to read only 1 bit from sensor
...
binary_decimal:
	...
	lds r18,SCRATCHPAD + 7			;copy MSB of temprature value from SRAM to r18
...
checkCRC:
	...
	ldi r19,56					; total number of bits
	lds r16,SCRATCHPAD + 7		; copy LSB of ROM/result to r8
	mov r8,r16			
	lds r16,SCRATCHPAD + 6		; copy LSB + 1 of ROM/result to r7
	mov r7,r16
	lds r16,SCRATCHPAD + 5		; copy LSB + 2 of ROM/result to r6
	mov r6,r16
	lds r16,SCRATCHPAD + 4		; copy LSB + 3 of ROM/result to r5
	mov r5,r16
	lds r16,SCRATCHPAD + 3		; copy LSB + 4 of ROM/result to r4
	mov r4,r16
	lds r16,SCRATCHPAD + 2		; copy LSB + 5 of ROM/result to r3
	mov r3,r16
	lds r16,SCRATCHPAD + 1		; copy LSB + 6 of ROM/result to r2
	mov r2,r16
	lds r16,SCRATCHPAD			; copy MSB of ROM/result to r1
	mov r1,r16
	ldi r17,0b10001100          ; polynomial 100110001 loaded as 00110001 (0x31) with msb bit discarded
	

CRCLOOP:
	clc					; clear carry
	lsr r1				; shift right msb,  doing right shifts will flip the scratch pad/rom result. ds18b20 transmit results lsb/lsbyte first and CRC last
	ror r2				; shift carry to msb of r2 and lsb to carry
	ror r3				; shift carry to msb of r3 and lsb to carry
	ror r4				; shift carry to msb of r4 and lsb to carry
	ror r5				; shift carry to msb of r5 and lsb to carry
	ror r6				; shift carry to msb of r6 and lsb to carry
	ror r7				; shift carry to msb of r7 and lsb to carry
	ror r8				; shift carry to msb of r8 (least significant byte) and lsbit of r8 to carry
	brcs XORIT			; if carry bit is set branch to label XORIT where a eor operation is performed on r8, carry set means the r8 shift out bit was 1
	dec r19				; decrease counter by 1
	brne CRCLOOP		; if r19 counter didnt reach 0 loop through above process over and over
	ret
XORIT:
	eor r8,r17			; if carry set we perform exclusive OR of data in r8 with polynomial 00110001 in r17
	dec r19				; decrease counter r19
	brne CRCLOOP		; if r19 counter didnt reach 0 loop to CRCLOOP process over and over
	ret
